...
/kaggle/input/362-trainzip/pill-faulty_imprint/212145.png
/kaggle/input/362-trainzip/grid-bent/1413653.png
/kaggle/input/362-trainzip/zipper-fabric_border/010861.png
/kaggle/input/362-trainzip/pill-crack/211461.png
/kaggle/input/362-trainzip/bottle-good/13696.png
/kaggle/input/362-trainzip/metal_nut-good/10106.png
/kaggle/input/362-trainzip/transistor-cut_lead/2412904.png
/kaggle/input/362-trainzip/hazelnut-good/11396.png
/kaggle/input/362-trainzip/capsule-crack/913455.png
/kaggle/input/362-trainzip/zipper-broken_teeth/612436.png
/kaggle/input/362-trainzip/toothbrush-good/11711.png
/kaggle/input/362-trainzip/carpet-thread/2410291.png
/kaggle/input/362-trainzip/tile-gray_stroke/713372.png
/kaggle/input/362-trainzip/capsule-squeeze/611613.png
/kaggle/input/362-trainzip/wood-good/13412.png
/kaggle/input/362-trainzip/pill-color/312189.png
/kaggle/input/362-trainzip/grid-thread/2714023.png
/kaggle/input/362-trainzip/grid-good/14260.png
/kaggle/input/362-trainzip/pill-combined/812808.png
/kaggle/input/362-trainzip/cable-cut_inner_insulation/2010777.png
/kaggle/input/362-trainzip/tile-glue_strip/1212930.png
/kaggle/input/362-trainzip/metal_nut-flip/1410132.png
/kaggle/input/362-trainzip/toothbrush-defective/814053.png
/kaggle/input/362-trainzip/leather-color/1313312.png
/kaggle/input/362-trainzip/hazelnut-hole/2213956.png
/kaggle/input/362-trainzip/cable-cut_outer_insulation/2811284.png
/kaggle/input/362-trainzip/leather-glue/1112341.png
/kaggle/input/362-trainzip/screw-thread_top/1011501.png
/kaggle/input/362-trainzip/zipper-fabric_interior/311455.png
/kaggle/input/362-trainzip/bottle-contamination/611273.png
/kaggle/input/362-trainzip/wood-liquid/2012106.png
/kaggle/input/362-trainzip/capsule-good/12204.png
/kaggle/input/362-trainzip/leather-fold/1313713.png
/kaggle/input/362-trainzip/cable-missing_cable/2710791.png
/kaggle/input/362-trainzip/tile-rough/1812096.png
/kaggle/input/362-trainzip/wood-combined/1312184.png
/kaggle/input/362-trainzip/pill-good/12956.png
/kaggle/input/362-trainzip/screw-thread_side/910019.png
/kaggle/input/362-trainzip/wood-hole/1112525.png
/kaggle/input/362-trainzip/wood-color/1013321.png
/kaggle/input/362-trainzip/screw-scratch_head/1210304.png
/kaggle/input/362-trainzip/leather-poke/812502.png
/kaggle/input/362-trainzip/capsule-scratch/713442.png
/kaggle/input/362-trainzip/zipper-good/13767.png
/kaggle/input/362-trainzip/metal_nut-bent/614150.png
/kaggle/input/362-trainzip/screw-manipulated_front/312344.png
/kaggle/input/362-trainzip/pill-contamination/1111251.png
/kaggle/input/362-test/21130.png
...
for category in tqdm(outlier1.index):
    os.makedirs('362_train/'+category, exist_ok=True) # macro f1 score / 88개의 class 구분, 데이터 격차가 큰 상태, outlier1이 가장 데이터가 적은 경우 > folder 이름 저장 필요(이미지를 늘렸을 때 구분) 362_train/으로 한번에 압축//exist_ok=True
    for i in range(35): #각 class에 해당하는 path
        for path in train[train['label']==category]['path']:
            ...
            cv2.imwrite('362_train/'+category+'/'+str(i)+path_split,augmented_image) #cv2.imwrite('362_train/'+category+'/'+path_split) 이미지가 중복 될 우려, 이미지가 만들어 질 때 새롭게 저장 필요 / path split에 .png 확장자 포함
shutil.make_archive('362_train','zip','362_train')#(파일이름,압축형태,폴더이름)
...
for category in tqdm(outlier2.index):
    os.makedirs('362_train/'+category, exist_ok=True) # macro f1 score / 88개의 class 구분, 데이터 격차가 큰 상태, outlier1이 가장 데이터가 적은 경우 > folder 이름 저장 필요(이미지를 늘렸을 때 구분) 362_train/으로 한번에 압축//exist_ok=True
    for i in range(25): #증식시키는 정도 range() #각 class에 해당하는 path
        for path in train[train['label']==category]['path']:
            image=cv2.imread(path)
            aug=al.Compose([al.HorizontalFlip(p=0.5),al.VerticalFlip(p=0.5),al.Rotate(limit=360,p=0.5),al.Resize(362,362)]) # for 문 밖에서도 가능
            aug_dict=aug(image=image)
...
for category in tqdm(outlier3.index):
    os.makedirs('362_train/'+category, exist_ok=True) # macro f1 score / 88개의 class 구분, 데이터 격차가 큰 상태, outlier1이 가장 데이터가 적은 경우 > folder 이름 저장 필요(이미지를 늘렸을 때 구분) 362_train/으로 한번에 압축//exist_ok=True
    for i in range(15): #각 class에 해당하는 path
        for path in train[train['label']==category]['path']:
            image=cv2.imread(path)
            aug=al.Compose([al.HorizontalFlip(p=0.5),al.VerticalFlip(p=0.5),al.Rotate(limit=360,p=0.5),al.Resize(362,362)]) # for 문 밖에서도 가능
            aug_dict=aug(image=image)
...
# good image size 줄이는 작업만 필요, 증식 x
...
from sklearn.model_selection import train_test_split # 이미지 > y가 필요하지 x
x_train, x_valid = train_test_split(train,test_size=0.2,random_state=42,stratify=train['label']) # stratify : 평가 셋을 뽑을 때도 트레인 같은 비율로 뽑기
```
...
```python
#test만들때 glob를 사용하지 않는 이유 : 순서가 뒤섞이면서 잘못 된 값으로 출력 id matching이 안되는 문제
test=pd.read_csv('/kaggle/input/dacon-cv-data/open/test_df.csv')
test
```
...
input에 test.zip 넣어준 후  ./362_test/ 변경 필요
...
2022-06-06 13:15:40.715673: W tensorflow/core/framework/cpu_allocator_impl.cc:80] Allocation of 25160448 exceeds 10% of free system memory.

2022-06-06 13:15:40.812599: I tensorflow/compiler/mlir/mlir_graph_optimization_pass.cc:185] None of the MLIR Optimization Passes are enabled (registered 2)
...
#class_mode 정답 칼럼이 숫자인지 문자인지, train에는 str로 있어서 True default로 사용할 수 있으나 test에는 없기 때문에
```
...
```python
result=model.predict(test_generator,verbose=1,workers=2) #workers를 2로 쓰면 gpu를 동시에 사용 가능
```
