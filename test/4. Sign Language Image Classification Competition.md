...
-------
label(정답값)은 1 ~ 9, 10-1, 10-2 총 11개로 분류됩니다.
각 label은 data 갯수가 균형있게 분포되어 있습니다.
주어진 image path를 이용하여 'path' column을 만들 수 있습니다.
...
-----
train data를 변형함으로써 학습 효과를 극대화하고 예측의 정확성을 높이려합니다.
수화 이미지는 손가락의 정확한 모양을 파악하는 것이 중요한데 이미지를 색상/명도/채도로 바꾸어 변환함으로써 촬영 장소의 밝기나 환경의 영향을 덜 받도록 했습니다.
용량을 줄여 속도를 빠르게 하고, 이미지를 절반의 크기로 잘라 모서리에 손가락이 나오는 경우를 대비하도록 했습니다.
하나의 손은 환경의 영향을 받을 수 있으나 대체로 같은 픽셀을 갖을 확률이 높기 때문에 특정 픽셀을 기준으로 흰색/검정색으로 나눔으로써 주변에 비해 손동작 부분을 구분하도록 했습니다.
무엇보다 손가락 끝부분의 모양에 따라 label이 달라지는 경우가 많으므로 캐니 경계선 감지 기법을 사용했습니다.
...
-------
주어진 데이터로 test의 file_name만을 이용하여 path column을 만들 수 있습니다.
...
또한, ImageNet에서 성능은 우수하나 처리 용량이 크지 않은 EfficientNetB1을 이용하여 Sequential 층을 쌓겠습니다. 또한 학습 개선이 없을 경우 중단하는 EarlyStopping과 learning rate를 조정함으로써 최적점을 찾는데 용이한 ReduceLROnPlateau를 callback함수로 이용하겠습니다.
...
-----
제출할 데이터에는 'label' column의 값이 모두 0으로 표기되어 있습니다.
result 값에서 1~9, 10-1, 10-2 각각에 해당하는 확률중 가장 높은 경우만을 반환하여 대입합니다.
...
--------
그러나 학습 및 예측 과정에서 10-1과 10-2를 2보다 앞의 순위로 인식하였고, 각 숫자에 대응하는 실제 값으로 변환하는 과정이 필요합니다.
