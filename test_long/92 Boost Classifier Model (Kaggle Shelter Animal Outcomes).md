...
----------
'AgeuponOutcome'연령의 단위가 다르게 있어 train과정에 어려움이 있을 것으로 보고, 모두 day로 변환합니다.
...
----
cat boost classifier에서 가장 정확도가 높은 alldata2 set 입니다.

다른 Boosting Model 혹은 Ensemble Model 사용의 경우 "DateTime" column이 drop되어야 오류가 발생하지않습니다.
...
---------
도표를 통해, adoption이 17~18시에 많기 때문에 출 퇴근 시간의 영향이 있음을 알 수 있습니다.

0~9, 19~24 구간에서 count의 갯수가 현저히 낮아지므로 사람의 여가 및 수면 시간과도 관계있음을 유추할 수 있습니다.

catboostclassifier를 이용하고, DateTime column을 삭제하였을때 score는 0.74015였으나 DateTime Column을 추가하니 score는 0.73824로 개선되었습니다.(score 낮을 수록 순위 개선) DateTime의 날짜형식을 인식할 수 있는 것이 CatBoostClassifier가 가지는 장점 중 하나임을 알게되었습니다.
...
-------
도표를 통해 adoption(입양)은 대부분 연령이 낮은 동물에게서 나타남을 알 수 있습니다. (연령과 분석의 target인  outcometype은 상관관계가 있습니다.)
...
----------------------
중성화 되었을 때, 다른 OutcomeType에 비해 입양비율이 현저히 높기 때문에,  train과정에 SexuponOutcome  column을 포함 시켜야합니다.

CatBoostModel에서 score 0.73824였던 조건에 SexuponOutcome 컬럼만 삭제하였더니 score 0.81490으로 변경되었습니다.
...
데이터 전처리 과정을 통해 학습에 적합한 column을 분별하였으니 Classifier Model의 적합성을 실험하고자 합니다.

CatBoostClassifier / RandomForestClassifier / GradientBoostingClassifier / XGBoost classifier / LightGBMclassifier 를 적용해보겠습니다.

1) CatBoostClassifier (score:0.74015 , score가 낮을수록 순위 향상됩니다.)
...
2) RandomForestClassifier score 0.83696
...
```
-----
3) GradientBoostingClassifier(score 0.76171)
...
-----
4) XGBoost classifier (score : 0.76171) 
...
5) lgbmclassifier (score 0.73234)
...
------------------------------------------------
LightGBM Classifier > CatBoosting Classifier > GradientBosstingClassifier = XGBoost Classifier > RandomForestClassifier 순으로 점수가 개선됨을 확인했습니다.

CatboostingClassifier가 RandomForestClassifier에 비해 정확도가 높은 이유는 오류가 발생할 경우 지속해서 보정하기 때문입니다.

또한, SanFranciscoCrimeClassifier는  1762311 rows × 6 columns / 이번 데이터는 38185 rows × 12 columns로 약 23배 데이터의 크기가 작아 train및 predict에 시간이 더 적게 들어 빠른 시간 안에 적용할 수 장점이 있습니다. 따라서 시간이 조금 더 소요되는 CatBoostingClassifier를 사용하더라도 이용에 불편함이 없습니다.

LightGBM Classifier가 CatBoosting Classifier에 비해 정확도가 높아 의구심이 들었는데, 트리 분할 과정에서 대칭성의 차이가 발생함을 알게되었습니다.

LightGBM Classifier는 트리의 균형을 맞추지 않고 최대 손실 값을 가지는 리프 노드를 지속적으로 분할하면서 트리의 깊이가 깊어지므로 균형트리 분할 방식(CatBoostingClassifier)보다 예측 오류 손실을 최소화할 수 있기 때문입니다.
