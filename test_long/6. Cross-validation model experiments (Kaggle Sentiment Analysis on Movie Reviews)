...
-------------
pretrained embedding : 방대한 양의 데이터로 미리 학습 된 것을 이용하여 다른 데이터의 embedding에 적용하는 것을 의미합니다.
...
```
--------------
#  검증 방식 중 KFold / StratifiedKFold / GroupKFold / LeaveOneOut 을 적용하여 장단점을 분석하고, 교차검증의 n_splits의 값 변화에 따른 영향성을 확인하겠습니다.

train2는 numpy의 array형식 / train은 pandas의 Series 형식이기 때문에 'iloc' 데이터에 접근하는 함수 사용이 필요합니다.
...
# 따라서 StraitifedKFold 모델을 사용하여 5개로 split, 교차 검증하는 것이 가장 적합한 것으로 보았습니다.
